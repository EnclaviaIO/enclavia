...
3. Dispatches signed `Control` commands (e.g. `PrepareUpgrade`) to [`enclavia-crypto`](../enclavia-crypto/) after verifying their control proof against the P-256 public key baked into the measured EIF config. Proofs may be legacy 64-byte raw P-256 signatures or versioned CTAP2/FIDO2 ES256 assertions with RP-ID, user-presence, user-verification, non-backup-eligible-credential, and proof-size checks. For FIDO2 commands, the envelope counter must advance beyond the inner chain-link proof's counter; the persisted chain then enforces monotonicity between commands. When the config carries a `min_upgrade_delay_secs`, a `PrepareUpgrade` whose activation time lands inside the delay window is rejected against the enclave's own clock.
