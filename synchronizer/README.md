...
- Raft state machine for the freshness map, follower-redirect on writes, hydrate-from-peers on restart, cold-start recovery (tracked as separate sub-issues). The Raft transport (slice 3) rides the mesh's `MeshPayload` envelope unchanged.
