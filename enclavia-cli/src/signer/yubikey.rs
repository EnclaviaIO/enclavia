...
fn prompt_pin(serial: u32) -> Result<String, CliError> {
    ...
    let pin =
        rpassword::read_password().map_err(|e| CliError::Other(format!("reading PIN: {e}")))?;
    if pin.is_empty() {
...
impl YubiKeySigner {
    ...
    pub fn open(serial: Option<u32>, slot: &str, public_key: [u8; 65]) -> Result<Self, CliError> {
...
pub fn generate_on_device(params: &GenerateParams<'_>) -> Result<(u32, [u8; 65]), CliError> {
    ...
    let spki = yubikey::piv::generate(&mut device, slot, AlgorithmId::EccP256, pin, touch)
        .map_err(|e| CliError::Other(format!("on-device key generation failed: {e}")))?;
...
fn sec1_from_point(point: &[u8]) -> Result<[u8; 65], CliError> {
    ...
    crate::keys::decode_public_key(&base64::engine::general_purpose::STANDARD.encode(sec1))?;
    Ok(sec1)
}
