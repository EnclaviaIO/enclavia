...
/// Sleep helper for the reconnect backoff: `tokio::time::sleep` natively,
/// a JS `setTimeout`-backed promise on wasm (no tokio runtime / timer
/// exists there — the transport runs on the JS microtask queue, see
/// `ws::spawn_transport`).
...
async fn sleep(dur: Duration) {
    ...
    let millis = dur.as_millis().min(i32::MAX as u128) as f64;
    let promise = js_sys::Promise::new(&mut |resolve, _reject| {
        let global = js_sys::global();
        // globalThis.setTimeout works in browsers, workers, and Node.
        let set_timeout: js_sys::Function =
            match js_sys::Reflect::get(&global, &JsValue::from_str("setTimeout")) {
                Ok(f) => f.unchecked_into(),
                Err(_) => {
                    // No timer available: resolve immediately rather than
                    // hang the reconnect path.
                    let _ = resolve.call0(&JsValue::NULL);
                    return;
                }
            };
        let _ = set_timeout.call2(&global, &resolve, &JsValue::from_f64(millis));
    });
