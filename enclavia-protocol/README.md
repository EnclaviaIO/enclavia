...
- **Custody** (`custody`): the shared verifier and encoding for legacy raw P-256 and versioned CTAP2/FIDO2 control proofs, including a pre-decode size bound, rejection of backup-eligible credentials, and signature-counter monotonicity helpers; the DER-to-raw low-S conversion for PIV; and the two-phase confirm/revoke signing DTOs shared by the CLI and the backend.
